...
async function loop() {
  ...
  while (true) {
    ...
    //    下面那道守卫查的是**公开课程目录**（未登录也拿得到），拦不住这一种。
    //    真正的判据：本站总学时要求恒为 90，**永远不可能是 0**。读到 0 就是没读到账号。
    if (!Object.keys(fin.req).length || !(fin.req['总学时']?.r > 0)) {
      log('❌ 平台没返回学时要求（登录态失效？），这不是"学完了"，2 分钟后重试');
      await sleep(120_000);
      continue;
    }
