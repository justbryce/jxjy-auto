...
touch state/DISABLED-zjsjczx   # 单独停用某个站（这个平台目标已达成，别再刷了）
rm state/DISABLED-zjsjczx      # 恢复。start.sh / watchdog.sh / setup-windows.mjs / heal / progress-check 都认这个标记
```
...
| `NC_HEAP_MAX_MB` | `900` | 网易云 tab 的 JS 堆超过这个值就整页重载。SPA 换课时不释放堆，不重载会一路涨到 4G 把渲染进程撑死（见坑 11d）|
...
> 🔴 **「学时重算」的后台页面（`/#/backIndex`）用 localStorage 判登录，不是 cookie。**
> 而 axios 的响应拦截器只要收到任何 `status == -1`（"请先登录"）就把
> `use_info` / `front_token` / `token` 一起删掉。于是会出现一个很迷惑的中间态：
> **cookie 还有效（`Auth.index/isLogin` 返回 1）、接口通、课照学、学时照记，
> 但后台页面进不去** —— 路由守卫把你悄悄弹回首页（URL 停在 `#/backIndex`，渲染的却是首页）。
> 只能**人在 Chrome 里点一次「学员登录」重新登录**。硬造 `use_info` 要猜字段结构，风险大于收益。
> `recalc.mjs` 现在会先查这个键并直接报出来，不再只说"找不到按钮"。
...
**5c. 🔴「一门课都没返回」≠「全学完了」—— 而"学完了"是个不可逆的决定。**
新干线的 runner 在登录态失效期间读到空的学时要求（全是 0）→ 调度器认为"什么都不缺"
→ 打印「🎉 没有可学的课了 / 收工，退出」并**结束进程**。
更糟的是 `watchdog.sh` 见到"收工"就**永远不再拉起它** —— 一次误判 = 永久停摆。
（2026-08-04：阿布正在重新登录，runner 已经收工走人了。）

原有的守卫查的是**公开课程目录**（未登录也拿得到），拦不住这一种。
现在两层都补上了：
- runner：**本站总学时要求恒为 90，永远不可能是 0**，读到 0 就是没读到账号，重试而不是收工
- watchdog：「收工就不再拉起」只认 6 小时，过期无条件复查一次（真学完的话它立刻再退出，代价可忽略）

**任何"终止"决定都要问：这个信号在"环境坏了"的时候会不会也出现？** 会的话就得有第二层兜底。
...
**11d. 🔥 网易云的播放页是 SPA，换课时不重载 → JS 堆只涨不落，最后把整台机器拖垮。**

实测跑 40 分钟后，4 个 worker 的 `performance.memory.usedJSHeapSize` 分别是
**1185 / 3765 / 1006 / 1335 MB**，而 Chrome 的堆上限是 4192 MB ——
那个 3765 MB 的 tab 已经**完全无响应**（`Runtime.evaluate` 超时），
它对应的渲染进程单独吃掉 1.7 GB 物理内存。

**这是"整机内存被刷爆"（坑 6h）的真正来源。** 之前一路降并发（10→5→3）其实是在治标：
并发只决定泄漏**多快**，不决定泄漏**存不存在**。

解法：换课时之前先读堆，超过阈值就**整页重载**（`NC_HEAP_MAX_MB`，默认 900）。
⚠️ SPA 内部跳转不释放堆，必须真的 `navigate` 一次 `about:blank` 再回来。
代价几乎为零 —— 163 本来就不认断点续播（坑 11），每个课时都是从 `currentTime=0` 重播的。
实测重载后堆从 1010~1663 MB 降到 201~362 MB，系统内存压力从等级 2 回落到 1。

**通用教训：长时间跑的自动化，"每轮换一个目标"的循环要问一句 —— 这一轮的内存还回去了吗？**
SPA 页面尤其危险，因为它看起来"换页了"，实际上从没卸载过。
...
**11e. 🔴 网易云播完会**自己跳到下一课时** —— 别把这个当成"页面跳走了"的错误。**
`watch()` 用「URL 里的 lessonId 还在不在」判断页面有没有被带跑。但视频播到头时
163 的 SPA **会自动切到下一课时**，lessonId 跟着变 —— 于是**成功被记成了失败**。

代价不在行为上（返回 retry 而已，不烧跳过计数，队列每轮从平台重建、已完成的自然被过滤），
**代价在可观测性上**：实测「✓ 播完」只占真实完成量的 1/4，剩下 3/4 全躲在「页面跳走了」里。
对账时看到"平台每小时 +26，runner 只声称完成 7 个"，第一反应必然是"在空转"——
排查半天才发现是日志在骗人。

现在按最后一次已知进度区分：≥90% 记「✓ 播完（平台自动跳到了下一课时）」，
不足 90% 才是真跳走，并且日志里带上当时的百分比。

**把成功记成失败，和把失败记成成功一样危险** —— 后者让你漏掉真问题，前者让你在真问题
出现时也以为"哦这个正常"。
