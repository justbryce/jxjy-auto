...
| `HZ_GX_TARGET` | `20` | 新干线先把公需（行业+一般）刷到多少学时，再转专业课程 |
...
**5. 🔥 两个"静默空转"型 bug，症状一模一样：进程活着、日志刷得飞快、CPU 也在忙，但真实学时零增长。**
...
⚠️ 前提是**机器不能锁屏 / 屏幕不能休眠** —— 一锁屏所有窗口都算 occluded，
又退回 hidden 状态（实测锁屏后 163 从 100% 掉到 ~84%）。要跑满 1x 就让机器保持解锁。
...
累计值要看 `validstudytime`——而 `validstudytime` **不是实时更新的**，学习途中查到的常常还是 0。
判"有没有在推进"用 `playTime`，判"完成没有"用「已计 + 本次」。
