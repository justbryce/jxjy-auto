...
// 🔌 AppleScript 熔断器。
// Chrome 的 AppleEvent 处理线程会整个卡死（表现：`tell application "Google Chrome"` 一律
// 报 -1712 AppleEvent 已超时，连 `open -a` 都失败），而**同一时刻 CDP 完全正常**。
// 这种时候每次 osascript 调用都要白等满超时（8~15s），runner 会被拖得寸步难行。
// 所以连续失败若干次就熔断一段时间，让所有依赖 CDP 的部分照常跑 —— 只是窗口摆不了、速度降级。
let osaFails = 0, osaOpenAt = 0;
const OSA_TRIP = 2, OSA_COOLDOWN = 10 * 60_000;
export function osaHealthy() { return !(osaFails >= OSA_TRIP && Date.now() - osaOpenAt < OSA_COOLDOWN); }
function osaResult(ok) {
  if (ok) { osaFails = 0; return true; }
  if (++osaFails >= OSA_TRIP) osaOpenAt = Date.now();
  return false;
}

export function activateChrome() {
  if (!osaHealthy()) return false;
  try {
    execSync(`osascript -e 'tell application "Google Chrome" to activate'`, { stdio: 'ignore', timeout: 8000 });
    return osaResult(true);
  } catch { return osaResult(false); }
}
